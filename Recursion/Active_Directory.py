...
def is_user_in_group(user, group):
    ...
    group_sup_groups = group.get_groups()
    group_users = group.get_users()
    
    if user in group_users:
        return True
    elif len(group_sup_groups) == 0:
        return False
    else:
        for subgroub in group_sup_groups:
            return is_user_in_group(user, subgroub)
